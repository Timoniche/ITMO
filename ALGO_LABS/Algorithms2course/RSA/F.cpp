//
// Created by Timoniche on 5/18/2019.
//
...
big_integer &big_integer::operator+=(big_integer const &rhs)
{
    ...
    std::deque<uint32_t> res(std::max(data_.size(), rhs.data_.size()));
    uint64_t carry = 0;
...
big_integer &big_integer::operator-=(big_integer const &rhs)
{
    ...
    std::deque<uint32_t> res(max->data_.size(), 0);
    bool borrow = false;
...
big_integer &big_integer::operator*=(big_integer const &rhs)
{
    ...
    std::deque<uint32_t> res(data_.size() + rhs.data_.size(), 0);
...
big_integer long_unsigned_division(big_integer const &l, big_integer const &r)
{
    ...
    std::deque<uint32_t> result(a.data_.size() - b.data_.size() + 1);
...
big_integer &big_integer::operator<<=(int rhs)
{
    ...
    toTwosComplement();
    std::deque<uint32_t> res(data_.size());
    uint32_t carry = 0;
    for (size_t i = 0; i < res.size(); i++)
    {
...
big_integer &big_integer::operator>>=(int rhs)
{
    ...
    uint32_t bigShift = static_cast<uint32_t>(rhs) / 32;
    uint32_t smallShift = static_cast<uint32_t>(rhs) % 32;
    std::deque<uint32_t> res(data_.size() - bigShift);
    uint64_t carry = 0;
...
int main()
{
    std::string s1, s2;
    std::cin >> s1;
    big_integer first(s1);
    std::cin >> s2;
    big_integer second(s2);
